# SPDX-FileCopyrightText: 2011-2014 Burak Beyhan
...
inputFile = "data/barnsbury_axial_small.shp"
outputFile = "data/results.shp"
localRadius = 3 # Radius for local
calcIntelligibility = TRUE # Calculate intellegibility value
...
if (shpfiled != "")
{
    ...
    calcDosya=function(){
        ...
        return(sf::st_read(shpfileds))
    }
    dosya = calcDosya()
    {
        dbfs = as.data.frame(sf::st_drop_geometry(dosya))
        bb = as.matrix(dbfs[5])
        bb = 1-bb/max(bb)
        if(any(is.na(bb)) || length(bb)==0 || any(is.nan(bb))) bb = 0
        plot(dosya, col=rgb(bb, 0.2, 0.4), lwd=3)
    }

    # calculation of intellegibility value

    if(calcIntelligibility) {
        print(paste("Intelligibility value: ", cor(st_drop_geometry(dosya)[, "Connectivi"],
                                                   st_drop_geometry(dosya)[, "GlobalInte"])))
    }
}
